...
export class ReportComponent  implements OnInit{
  ...
  draw(){
    this.consultService.callProcedureNative().subscribe(data=>{
  ...
  viewReport(){
    return this.consultService.generateReport().subscribe(data=>{
  ...
  convertTo64(data:any){
    const reader = new FileReader();
    reader.readAsDataURL(data);
    reader.onloadend= ()=>{
      const base64 = reader.result;
      //this.imageData= base64;
      this.applySanitazer(base64);
    }
  }

  applySanitazer(data:any){
    this.imageData= this.sanitazer.bypassSecurityTrustResourceUrl(data);
